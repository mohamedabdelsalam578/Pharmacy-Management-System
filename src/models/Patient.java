...
public class Patient extends User {
    ...
    private Order cartOrder; // Current shopping cart
    ...
    public Patient(int id, String name, String username, String password, 
        ...
        super(id, username, password, name, UserRole.PATIENT);
    ...
    public Patient(int id, String username, String passwordHash, String firstName, String lastName,
        ...
        super(id, username, passwordHash, firstName + " " + lastName, UserRole.PATIENT);
    ...
    /**
     * Calculate and get the age of this patient based on the date of birth
     * 
    ...
    public int getAge() {
        ...
        if (birthCalendar.get(Calendar.MONTH) > currentCalendar.get(Calendar.MONTH) || 
            (birthCalendar.get(Calendar.MONTH) == currentCalendar.get(Calendar.MONTH) && 
             birthCalendar.get(Calendar.DAY_OF_MONTH) > currentCalendar.get(Calendar.DAY_OF_MONTH))) {
            age--;
        }
    ...
    /**
     * Get contact information (phone and email) formatted for display
     * 
     ...
     * 
     * @return The shopping cart order
     */
    public Order getCartOrder() {
        // Ensure the cart is never null for access from other classes
        // Lazy initialization if needed
        if (cartOrder == null) {
            System.out.println("Creating new cart for patient " + getName() + " (id: " + getId() + ")");
            cartOrder = new Order(0, getId());
        }
        return cartOrder;
    }
     ...
     * 
     * @param cartOrder The shopping cart order
     */
    ...
    /**
     * Checkout the current cart (add to orders and reset cart)
     * 
     * @return The completed order
     */
    public Order checkoutCart() {
        // Only if cart has items
        if (cartOrder != null && !cartOrder.getItems().isEmpty()) {
            // Give it a proper ID if needed
            if (cartOrder.getId() == 0) {
                ...
                for (OrderItem item : cartOrder.getItems()) {
            ...
            // Add to orders list
            this.orders.add(cartOrder);
            // Create fresh cart
            Order completedOrder = cartOrder;
            cartOrder = new Order(0, getId());
